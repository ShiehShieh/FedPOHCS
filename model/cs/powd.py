...
class PowerOfChoice(cs_lib.ClientSelectionBase):

  def __init__(self, clients, seed, **kwargs):
    super(PowerOfChoice, self).__init__(clients, seed)

  def select_clients(self, num_clients, indices_cands):
    num_clients = min(num_clients, len(indices_cands))
    indices = np.argsort(self.objs)
    # Selecting clients with smallest returns.
