...
class Agent(object):
  ...
  def eval(self, observations, actions, advantages):
    return self.policy.eval(observations, actions, advantages)
  ...
  def epsilon_greedy(self, observations):
    actions, probs = self.act(observations)
    # epsilon-greedy exploration strategy
    if random.random() < self.epsilon:
      if len(actions.shape) == 0:
        return random.randint(
            0, self.policy.num_actions - 1, size=actions.shape[0]), probs
      return np.random.rand(*actions.shape), probs
    else:
  ...
  def adapt_sgd_learning_rate(self, num):
    self.policy.adapt_sgd_learning_rate(num)
