...
class TRPOActor(pg_lib.PolicyGradient):
  def __init__(self,
    ...
    with self.graph.as_default():
      self.beta = tf.Variable(initial_value=beta, dtype=tf.float64)
      self.sigma = tf.Variable(initial_value=sigma, dtype=tf.float64)
      self.mu = tf.Variable(initial_value=mu, dtype=tf.float64)
      self.ent_coef = tf.Variable(initial_value=ent_coef, dtype=tf.float64)
      ...
      with tfv1.variable_scope(model_scope, default_name='trpo') as vs, tf.GradientTape(persistent=True) as gt:
        ...
        self.advantages = tfv1.placeholder(
            tf.dtypes.float64, [None], name='advantages')
        self.sampled_prob = tfv1.placeholder(
            tf.dtypes.float64, [None, self.num_actions], name='probs')
        self.dropout_pd = tfv1.placeholder(tf.float64, name='dropout_rate')
        self.state_visitation_frequency = tfv1.placeholder(
            tf.float64, name='state_visitation_frequency')
        self.norm_penalty = tfv1.placeholder(
            tf.float64, name='norm_penalty')
        ...
        if env.is_continuous:
          ...
          self.sampled_prob = tfv1.placeholder(
              tf.dtypes.float64, [None, self.num_actions * 2],
              name='probs')
        else:
    ...
    with self.graph.as_default():
      ...
      self.base_lr = self.sess.run(optimizer.learning_rate)
  ...
  def build_loss(self, actions, prob_pi, prob_old, prob_anchor):
    ...
    # Entropy bonus.
    ent = tf.reduce_mean(
        self.prob_type.entropy(prob_pi)
    )
    et_bns = self.ent_coef * ent
    ...
    return -(surr - kl_pen - nm_pen + et_bns) + px_pen

  def build_opti_ops(self, obj, pi_var, grad_tape):
    # compute gradients
    pi_grad = tf.gradients(obj, pi_var)
  ...
  def proximal_term(self, n1, n2):
    v1 = n1.var()
    v2 = n2.var()
    norm = tf.zeros((), dtype=tf.float64)
  ...
  def adapt_kl_penalty_coefficient(self, kl, scale):
    ...
    if kl < self.kl_targ / threshold:
      ...
      beta = beta * step
    # The abs is needed here because this value kl < 0 occasionally.
    beta = min(min(beta, np.abs(scale / kl)), 20.0)
    beta = max(beta, 1e-8)
  ...
  def adapt_entropy_coefficient(self, num):
    coef = self.sess.run(self.ent_coef)
    if 0.0 < self.ent_decay < 1.0:
      coef = coef * self.ent_decay
    elif (num + 1) % self.ent_decay == 0:
      coef = coef / 10.0
    # Set minimum entropy coefficient.
    coef = max(coef, 1e-8)
    self.ent_coef.load(coef, self.sess)

  def adapt_sgd_learning_rate(self, num):
    self.adapt_entropy_coefficient(num)
    # # Decay by half every 10 rounds.
    # p = num // 10.0
    # mu = self.base_lr / (2 ** p)
    # self.optimizer.learning_rate.load(mu, self.sess)
  ...
  def adapt_kl_target(self, num):
    s, e, span = self.kl_targ_adap
    i = (e - s) / span * num
    if s > e:
      self.kl_targ = max(s + i, e)
    else:
      self.kl_targ = min(s + i, e)
  ...
  def stat(self):
    lr, beta, sigma, ent_coef = self.sess.run(
        [self.optimizer.learning_rate, self.beta, self.sigma,
         self.ent_coef])
    msg = {
        'lr': lr,
        'ent_coef': ent_coef,
        ...
        'avg_gn': self.avg_gn,
        # 'avg_ad': self.avg_ad,
        'nm_targ': self.nm_targ,
    }
    if np.isnan(self.avg_kl):
      raise Exception("NaN encountered: avg_kl. msg: %s" % (msg))
    return msg

  def eval(self, observations, actions, advantages):
    m = {
    ...
    obj, grad_norm = self.sess.run(
        [self.obj, self.grad_norm], feed_dict=m)
    return obj, grad_norm
  ...
  def fit(self, steps, logger=None):
    ...
    self.max_et = np.max(self.log_et)
