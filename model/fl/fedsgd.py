...
class FedSGD(fedbase_lib.FederatedBase):
  ...
  def finalize_clients(self):
    ...
    for client in self.clients:
      # client.num_iter = 1
      client.agent.policy.num_epoch = 1
      self.timestep_per_batch = 999 # MCC.
      self.timestep_per_batch = 1000 # Swimmer.
      client.agent.policy.batch_size = self.timestep_per_batch
    # Full clients participation.
    self.num_cands = len(self.clients)
    self.clients_per_round = len(self.clients)
