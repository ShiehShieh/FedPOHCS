...
class FedTRPO(fedbase_lib.FederatedBase):
  ...
  def _inner_vectorized_loop(self, i_iter, indices, retry_min):
    ...
    #   # NOTE(XIE,Zhijie): The setting for the submitted paper.
    #   # c.set_nm_penalty_coefficient(self.sigma)
    #   c.adapt_nm_target(i_iter)
    ...
    self.retry(
        ...
        lambda: self.universial_client.experiment(
            num_iter=self.num_iter,
            timestep_per_batch=self.timestep_per_batch, indices=indices,
            agents=self.agents, obfilts=self.obfilts,
            rewfilts=self.rewfilts,
            callback_before_fit=[c.sync_old_policy
                                 for c in active_clients],
            logger=logger,
        ),
