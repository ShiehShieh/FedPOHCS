...
def truncate_trajectories(trajectories, l):
  keys = list(trajectories[0].keys())
  return [{k: t[k][:l] for k in keys} for t in trajectories]
...
def tf_shuffle(lists):
  f = lists[0]
  indices = tf.range(start=0, limit=tf.shape(f)[0], dtype=tf.int32)
  shuffled_indices = tf.random.shuffle(indices)
  return [tf.gather(l, shuffled_indices) for l in lists]

def stablize(x):
  eps = 1e-8
  return tf.cast(tf.math.less_equal(x, eps), tf.float64) * eps + x
