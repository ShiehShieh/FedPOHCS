...
class DiagGauss(ProbType):
    ...
    def kl(self, prob0, prob1):
      mean0 = prob0[:, :self.d]
      std0 = tf.maximum(prob0[:, self.d:], 1e-5)
      var0 = tf.square(std0)
      mean1 = prob1[:, :self.d]
      std1 = tf.maximum(prob1[:, self.d:], 1e-5)
      var1 = tf.square(std1)
      ...
      def _one_dim_kl(mu1, sigma1, stddev1, mu2, sigma2, stddev2):
        ...
        t1 = tf.math.log(stddev2 / stddev1)
        t2 = (sigma1 + tf.math.square(mu1 - mu2)) / (2.0 * sigma2)
      ...
      # return tf.reduce_sum(tf.math.log(std1) - tf.math.log(std0), axis=1) + tf.reduce_sum((tf.math.square(std0) + tf.math.square(mean0 - mean1)) / (2.0 * tf.math.square(std1)), axis=1) - 0.5 * self.d
    ...
    def entropy(self, prob):
        std_nd = prob[:, self.d:]
        return tf.reduce_sum(tf.math.log(std_nd), axis=1) + .5 * np.log(2 * np.pi * np.e) * self.d
