...
class DistributedBase(object):
  ...
  def __init__(self, clients_per_round, num_rounds, num_iter,
               ...
               parti_history_fn='', cs='random', seed=0):
    self.clients = []
  ...
  def register(self, client):
    self.clients.append(client) if self.global_weights is None:
  ...
  def train(self):
    ...
    for i in range(self.num_rounds):
      ...
      if i % self.eval_every == 0:
        ...
        norm_das = stats[4]
        norm_avgs = stats[5]
        da_b = [norm_das[i] - norm_bs[i] for i in range(len(norm_bs))]
        ...
        outer_loop.write(
            'At round {} expected future discounted reward: {}, averaged level of heterogeneity B norm: {}; averaged [D]xA norm: {}; norm of \mean DxA: {}; [D]xA - B: {}; # retry so far {}'.format(
                i, np.mean(rewards), np.mean(norm_bs), np.mean(norm_das),
                np.mean(norm_avgs), da_b, self.get_num_retry()),
            file=sys.stderr)

      cs_class = random_lib.Random
      if self.cs == 'csh':
        cs_class = csh_lib.CSH
      if self.cs == 'csh-reverse':
        cs_class = csh_lib.CSHReverse
      indices, selected_clients = cs_class(
          da_b, self.clients, self.seed * self.num_rounds
          ).select_clients(
