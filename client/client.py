...
class Client(object):
  def __init__(
    ...
    self.agent = agent
    self.env = env
    self.num_timestep_for_dynamics = 0
    self.ism = defaultdict(float)
    ...
    # logging.error(self.get_initial_state_distribution())
    # logging.error(self.get_transition_probability()[(0.0, 0.0)][(0.0,)])
    # logging.error(self.get_transition_probability()[(0.0, 0.0)][(-0.8,)])
    # logging.error(self.get_reward_function()[(0.0, 0.0)][(0.0,)])
    # logging.error(self.get_reward_function()[(0.0, 0.0)][(-0.8,)])
    # _, prob = self.agent.act([(1.2, 0.2,)])
    # logging.error(prob)
    envs.close()
  ...
  def adapt_sgd_learning_rate(self, num):
    self.agent.adapt_sgd_learning_rate(num)

  def adapt_kl_target(self, num):
    self.agent.adapt_kl_target(num)
    ...
    while True:
      ...
      while True:
        ...
        for i, trajectory in enumerate(trajectories):
          ...
          # total_rewards[i] += (self.agent.policy.future_discount ** len(trajectory['reward'])) * rewards[i]
        ...
        if np.all(already_done):
          # NOTE(XIE,Zhijie): Do not use np.all(dones) here, as some envs
  ...
  def update_initial_state_distribution(self, paths):
    return svf_lib.update_initial_state_distribution(self.ism, paths)

  def update_transition_probability(self, paths):
    for i, trajectory in enumerate(paths):
      self.num_timestep_for_dynamics += len(trajectory['observations'])
    return svf_lib.update_transition_probability(self.tpm, paths)

  def update_reward_function(self, paths):
    return svf_lib.update_reward_function(self.rm, paths)

  def get_initial_state_distribution(self):
    return svf_lib.get_initial_state_distribution(self.ism)

  def get_transition_probability(self):
    return svf_lib.get_transition_probability(self.tpm)

  def get_reward_function(self):
    return svf_lib.get_reward_function(self.rm)

  def prepare(self, logger=None):
    self.ism = defaultdict(float)
    self.tpm = defaultdict(lambda:defaultdict(lambda:defaultdict(float)))
    self.rm = defaultdict(lambda: defaultdict(lambda: [0.0, 0.0]))
    for paths in self.last_paths:
      self.update_initial_state_distribution(paths)
      self.update_transition_probability(paths)
      self.update_reward_function(paths)

    funcs = {
        'initial_state_distribution': self.get_initial_state_distribution(),
        'transition_probability': self.get_transition_probability(),
        'reward_function': self.get_reward_function(),
        'vf': self.vf,
    }
    info =  svf_lib.prepare(
        self.get_transition_probability(),
        self.agent.policy, logger)
  ...
  def get_da(self, info):
    ...
    d, a, vf = svf_lib.find_svf_v2(
        self.get_initial_state_distribution(),
        self.get_transition_probability(),
  ...
  def eval(self):
    # TODO(XIE,Zhijie): These trajectories may be outdated. Update them?
    objs = []
    grad_norms = []
    for paths in self.last_paths:
      steps = utils_lib.convert_trajectories_to_steps(paths)
      obss = steps['observations']
      acts = steps['actions']
      advs = steps['advantages']
      for idx in range(max(len(obss) // 512, 1)):
        start = idx * 512
        end = start + 512
        if idx == max(len(obss) // 512, 1) - 1 and end > len(obss):
          # NOTE(XIE,Zhijie): Drop the last batch if it is not full.
          break
        obj, grad_norm = self.agent.eval(
            obss[start:end], acts[start:end], advs[start:end])
        objs.append(obj)
  ...
  def test(self, eval_heterogeneity=False):
    ...
    envs.close()
    return np.mean(episode_rewards), np.mean([len(e['reward']) for e in paths])
