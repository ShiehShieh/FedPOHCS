...
def pyx_library(
    ...
    native.genrule(
        ...
        # cmd = "cd $$(dirname $(execpath setup.py)); PYTHONHASHSEED=0 python setup.py build_ext && mkdir -p $$(dirname $(OUTS)); mv `find build | grep so` $(OUTS)",
        cmd = "PYTHONHASHSEED=0 python $(execpath setup.py) build_ext && mv `find build | grep so` $(OUTS)",
              # Rename outputs to expected location.
              # TODO(robertwb): Add an option to cythonize itself to do this.
              # + """ && python -c 'import os, sys, logging; logging.error(sys.argv); n = len(sys.argv); [os.rename(src.split(".")[0] + ".cpython-36m-x86_64-linux-gnu.so", dst) for src, dst in zip(sys.argv[1:], sys.argv[1+n//2:])]' $(SRCS) $(OUTS)""",
        tools = ["@cython//:cythonize"] + pxd_srcs + [setup_py],
    )
